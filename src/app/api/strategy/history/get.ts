...
export async function GET(request: NextRequest) {
  ...
  try {
    let response
    if (goalId) {
      response = await strategyHistoryHandler.findManyByGoalId(goalId, { strategyId, planId, sequence }, sequenceStr)
    } else {
      response = await strategyHistoryHandler.findMany({ strategyId, planId, sequence }, sequenceStr)
    }
